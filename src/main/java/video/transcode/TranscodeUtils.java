...
//	private long begintime;
	...
	public TranscodeUtils(String filename) {
		targetfolder = targetfolder + filename;
		File file = new File(targetfolder);
		if (!file.exists()){
			file.mkdir();
		}
		imageRealPath = targetfolder + "\\img";
	}
	...
	private String sumTime(long timecha) {
		Date date = new Date(timecha);
...
//          }
	...
	private boolean processFLV(String oldfilepath) {
		...
		commend.add("24");
		commend.add("-y");
		commend.add(targetfolder + filerealname+ "\\" +filerealname + ".flv");
		try {
			ProcessBuilder builder = new ProcessBuilder();
			String cmd = commend.toString();
			builder.command(commend);
